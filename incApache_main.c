...
/*** TO BE DONE 4.0 START ***/
	if (chroot(www_root)){	//eseguo chroot e controllo se ci sono errori
		fprintf(stderr, "ERROR chroot: %s\n", strerror(errno));
	}
	create_listening_socket(port_as_str);
	
	drop_privileges();	//per droppare i privilegi
/*** TO BE DONE 4.0 END ***/
#ifdef INCaPACHE_4_1
...
/*** TO BE DONE 4.0 START ***/
	void* tmp = client_connection_thread(&connection_no[i]);	 //i NULL sono gli attributi (con NULL sono al valore di default)
	if (tmp == NULL) printf("client_connection_thread failed\n");
	thread_ids[i] = &tmp;
/*** TO BE DONE 4.0 END ***/
