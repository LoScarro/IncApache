...
/*** TO BE DONE 4.0 START ***/
	file_size = (unsigned long)(stat_p->st_size);
...
/*** TO BE DONE 4.0 START ***/
	struct stat tmp;
	if(access(HTML_404,R_OK) != 0){
		fail_errno("errore accesso file 404:");		
	}
	(void)stat(HTML_404,&tmp);
	file_size=(unsigned long)(tmp.st_size);
	mime_type=(char*)(malloc(strlen(HTML_mime)));
	strcpy(mime_type,HTML_mime);
	file_modification_time=tmp.st_mtime;
/*** TO BE DONE 4.0 END ***/
...
/*** TO BE DONE 4.0 START ***/
	struct stat tmp;
	if(access(HTML_501,R_OK) != 0){
		fail_errno("errore accesso al file 501:");		
	}
	(void)stat(HTML_501,&tmp);
	file_size=(unsigned long)(tmp.st_size);
	mime_type=(char*)(malloc(strlen(HTML_mime)));
	strcpy(mime_type,HTML_mime);
	file_modification_time=tmp.st_mtime;
...
/*** TO BE DONE 4.0 START ***/
	char a[] = {"\r\nSet-Cookie: UserID="}; // prima stringa

	// seconda stringa
	int x = cookie;
	int length = snprintf(NULL, 0, "%d", x);
	char* b = malloc(length + 1);
	snprintf(b, length + 1, "%d", x);
	// printf("cookie in stringa: %s\n", str); // DEBUG

	// terza stringa
	char c[] = {"; Expires=Tue, 29 Dec 2021 23:59:59 GMT"};
	...
	char *s_cookie = malloc(alen + blen + clen + 1);
	if(s_cookie)
	{
		memcpy(s_cookie, a, alen);
		memcpy(s_cookie + alen, b, blen);
		memcpy(s_cookie + alen + blen, c, clen + 1);
	}
	else 
		printf("malloc failed\n");

	// prende la stringa modificata e la mette dentro header
	strcat(http_header, s_cookie); 
/*** TO BE DONE 4.0 END ***/
...
/*** TO BE DONE 4.0 START ***/
	strftime(time_as_string,40,"%a, %d %b %Y %X %Z", gmtime(&file_modification_time));
/*** TO BE DONE 4.0 END ***/
...
/*** TO BE DONE 4.0 START ***/
		option_val = malloc(n*sizeof(size_t));		//n contiene la size di http_request_line
		if (option_val==NULL)
			printf("malloc failed\n");
		option_name = malloc(n*sizeof(size_t));
		if (option_name==NULL)
			printf("malloc failed\n");
		option_name = strtok(strtokr_save, "=");
		option_val = strtok(NULL, "\r\n");
		UIDcookie=atoi(option_val);		// converto la stringa in un intero
		...
		if(option_val != NULL)
			UIDcookie = atoi((char*)memchr(option_val, '=', strlen(option_val))+1);
		/*** TO BE DONE 4.0 END ***/
                            }
...
/*** TO BE DONE 4.0 START ***/
		...
		if (strptime(http_option_line,"%a, %d %b %Y %T GMT",&since_tm)==NULL)		// converto la data contenuta in http_option_line in una struct tm
...
/*** TO BE DONE 4.0 END ***/
		...
		if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
			...
			if (MethodIsConditional(http_method)) {
				 ...
				 ***/
/*** TO BE DONE 4.0 START ***/
		if(my_timegm(&since_tm) >= stat_p->st_mtime){		// se la versione del client è aggiornata rispondo not modified 304
